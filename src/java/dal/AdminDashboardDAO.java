...
public class AdminDashboardDAO extends DBContext {
    ...
    /**
     * Retrieves a full KPI summary including total users, orders, products, and
     * policies.
     */
    public DashboardSummary getDashboardSummary() throws Exception {
        ...
        try (Connection con = getConnection()) {
            ...
            PreparedStatement ps2 = con.prepareStatement("SELECT COUNT(*) FROM [Order]");
            ResultSet rs2 = ps2.executeQuery();
            if (rs2.next()) {
                summary.setTotalOrders(rs2.getInt(1));
            }

            PreparedStatement ps3 = con.prepareStatement("SELECT COUNT(*) FROM Product");
            ResultSet rs3 = ps3.executeQuery();
            if (rs3.next()) {
                summary.setTotalProducts(rs3.getInt(1));
            }

            PreparedStatement ps4 = con.prepareStatement("SELECT COUNT(*) FROM WarrantyPolicies");
