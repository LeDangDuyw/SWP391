...
<% 
    ...
    if (products == null) {
        products = new ArrayList<Product>(); // Phòng trường hợp thuộc tính null không bị lỗi 500
    }
%>
<body class="bg-background text-on-surface font-body-md min-h-screen flex">

    <nav class="fixed left-0 top-0 h-full w-64 bg-surface-container border-r border-outline-variant/20 flex flex-col py-unit space-y-2 z-40">
        <div class="px-6 py-4 mb-4">
            <h1 class="font-headline-md text-headline-md font-bold text-primary">UNILAP Admin</h1>
            <p class="font-body-sm text-body-sm text-on-surface-variant">System Controller</p>
        </div>
        <a class="flex items-center px-4 py-3 mx-2 rounded-full text-on-surface-variant hover:bg-surface-container-highest transition-colors font-label-md text-label-md" href="#">
            <span class="material-symbols-outlined mr-3">dashboard</span> Dashboard
        </a>
        <a class="flex items-center px-4 py-3 mx-2 rounded-full text-on-surface-variant hover:bg-surface-container-highest transition-colors font-label-md text-label-md" href="#">
            <span class="material-symbols-outlined mr-3">shopping_bag</span> Orders
        </a>
        <a class="flex items-center px-4 py-3 mx-2 rounded-full bg-secondary-container text-on-secondary-container font-label-md text-label-md" href="#">
            <span class="material-symbols-outlined icon-fill mr-3">inventory_2</span> Inventory
        </a>
        <a class="flex items-center px-4 py-3 mx-2 rounded-full text-on-surface-variant hover:bg-surface-container-highest transition-colors font-label-md text-label-md" href="#">
            <span class="material-symbols-outlined mr-3">group</span> Users
        </a>
        <a class="flex items-center px-4 py-3 mx-2 rounded-full text-on-surface-variant hover:bg-surface-container-highest transition-colors font-label-md text-label-md" href="#">
            ...
            <div class="flex flex-col gap-4 mb-8">
                <div class="bg-surface border border-outline-variant/30 rounded-xl p-4 flex flex-wrap items-center gap-4 shadow-sm">
                    <div class="relative flex-1 min-w-[300px]">
                        ...
                        <input type="text" placeholder="Search inventory by product name, SKU, or category..." class="w-full pl-12 pr-4 py-2.5 bg-surface-container-low border border-outline-variant/50 rounded-lg font-body-sm text-body-sm text-on-surface focus:ring-2 focus:ring-primary/20 focus:border-primary placeholder-on-surface-variant/60 transition-all">
                    </div>

                    <div class="flex items-center gap-2">
                        <button class="flex items-center gap-2 px-4 py-2.5 bg-surface border border-outline-variant/50 rounded-lg hover:bg-surface-container-high transition-colors text-on-surface font-label-md text-label-md">
                        ...
                        <div class="relative">
                            <select class="appearance-none pl-4 pr-10 py-2.5 bg-surface border border-outline-variant/50 rounded-lg hover:bg-surface-container-high transition-colors text-on-surface font-label-md text-label-md focus:ring-0 focus:border-primary outline-none">
                    ...
                    <div class="flex items-center gap-2 ml-auto">
                        ...
                        <button class="px-4 py-2.5 bg-primary text-on-primary rounded-lg hover:bg-primary/90 transition-colors font-label-md text-label-md flex items-center gap-2 shadow-sm">
                ...
                <div class="flex flex-wrap items-center gap-2 px-1">
                    <span class="text-body-sm text-on-surface-variant mr-2">Active filters:</span>
                    <div class="flex items-center gap-1.5 px-3 py-1 bg-secondary-container/50 text-on-secondary-container rounded-full border border-outline-variant/20 text-[12px] font-bold">
                        Category: Laptops
                    ...
                    <button class="text-[12px] font-bold text-primary hover:underline ml-2 transition-all">Clear all</button>
                    <div class="ml-auto font-body-sm text-body-sm text-on-surface-variant">
                        Showing <span class="font-bold text-on-surface">1-10</span> of 124 items
                    </div>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-on-surface text-on-primary font-label-md text-label-md">
                                ...
                                <th class="py-3 px-4 border-b border-outline-variant/20">Price</th>
                        ...
                        <tbody class="font-body-sm text-body-sm">
                            <%-- VÒNG LẶP ĐƯỢC ĐƯA VÀO ĐÂY ĐỂ CHỈ LẶP LẠI DÒNG (TR) --%>
                                ...
                                <td class="py-2 px-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-on-surface-variant text-[12px]">M2 Max, 32GB RAM, 1TB SSD</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-2 px-4 font-code-sm text-code-sm text-on-surface-variant">ULP-14-M2-32-1T</td>
                                <td class="py-2 px-4">Laptops</td>
                                <td class="py-2 px-4 font-bold">$2,499.00</td>
                                <td class="py-2 px-4">
                            ...
                            <% } %> <%-- ĐÓNG VÒNG LẶP ĐÚNG CHỖ --%>
                        </tbody>
                    </table>
                </div>
                
                <div class="bg-surface px-4 py-3 border-t border-outline-variant/30 flex items-center justify-between">
                    <button class="px-3 py-1 border border-outline-variant rounded text-on-surface-variant font-label-md text-label-md hover:bg-surface-container-low disabled:opacity-50" disabled="">Previous</button>
                    <div class="flex gap-1">
                        <button class="w-8 h-8 flex items-center justify-center rounded bg-primary text-on-primary font-label-md text-label-md">1</button>
                        <button class="w-8 h-8 flex items-center justify-center rounded text-on-surface hover:bg-surface-container-low font-label-md text-label-md">2</button>
                        <button class="w-8 h-8 flex items-center justify-center rounded text-on-surface hover:bg-surface-container-low font-label-md text-label-md">3</button>
                        <span class="w-8 h-8 flex items-center justify-center text-on-surface-variant">...</span>
                        <button class="w-8 h-8 flex items-center justify-center rounded text-on-surface hover:bg-surface-container-low font-label-md text-label-md">13</button>
                    </div>
                    <button class="px-3 py-1 border border-outline-variant rounded text-on-surface-variant font-label-md text-label-md hover:bg-surface-container-low">Next</button>
                </div>
            </div>
