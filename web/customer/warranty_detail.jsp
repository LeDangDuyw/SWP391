...
<head>
    ...
    <title>Yêu cầu bảo hành #${selectedClaim.claimId} – UNILAP</title>
...
<div class="page-wrap">
    ...
    <div class="breadcrumb">
        <a href="${pageContext.request.contextPath}/warranty?action=list">
            <i class="fas fa-shield-alt" style="margin-right:4px;"></i>Trung tâm bảo hành
        </a>
        <span>/</span>
        <span>Yêu cầu bảo hành #<c:out value="${selectedClaim.claimId}"/></span>
    </div>
    ...
    <div class="detail-header">
        <div class="detail-header-left">
            ...
            <div class="meta">
                Yêu cầu bảo hành #${selectedClaim.claimId}
                &nbsp;·&nbsp;
        ...
        <span class="badge badge-${selectedClaim.status}">
            <c:choose>
                <c:when test="${selectedClaim.status == 'PENDING'}">Chờ xử lý</c:when>
                <c:when test="${selectedClaim.status == 'PROCESSING'}">Đang xử lý</c:when>
                <c:when test="${selectedClaim.status == 'APPROVED'}">Đã duyệt</c:when>
                <c:when test="${selectedClaim.status == 'REJECTED'}">Đã từ chối</c:when>
                <c:when test="${selectedClaim.status == 'COMPLETED'}">Đã hoàn thành</c:when>
                <c:when test="${selectedClaim.status == 'CANCELLED'}">Đã hủy</c:when>
                <c:otherwise>${selectedClaim.status}</c:otherwise>
            </c:choose>
        </span>
    </div>
                            ...
                            <c:forEach var="h" items="${selectedHistory}">
                                <div class="history-item">
                                    <div class="history-item-header">
                                        <span class="badge badge-${h.repairStatus}">
                                            <c:choose>
                                                ...
                                                <c:otherwise>${h.repairStatus}</c:otherwise>
                                            </c:choose>
                                        </span>
