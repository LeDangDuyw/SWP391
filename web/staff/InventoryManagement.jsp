        ...
        <main class="flex-1 p-gutter bg-surface-container-lowest">
            ...
            <div id="tab-products" class="<%= activeTab.equals("products") ? "" : "hidden" %>">
                <div class="bg-surface border border-outline-variant/50 rounded-lg overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="font-body-sm text-body-sm">
                                ...
                                <% if (product.isEmpty()) { %>
                                <tr>
                                    <td class="py-4 px-4 text-center text-on-surface-variant font-body-sm text-body-sm" colspan="4">
                    ...
                    <!-- Pagination Footer for Products Tab -->
                    <div class="bg-surface px-4 py-3 border-t border-outline-variant/30 flex items-center justify-between">
                        <c:set var="queryParams" value="&tab=products" />
                        <c:if test="${not empty searchInput}"><c:set var="queryParams" value="${queryParams}&searchInput=${searchInput}" /></c:if>
                        <c:if test="${not empty category}"><c:set var="queryParams" value="${queryParams}&category=${category}" /></c:if>
                        <c:if test="${not empty sortBy}"><c:set var="queryParams" value="${queryParams}&sortBy=${sortBy}" /></c:if>
                        <c:if test="${not empty itemStatus}"><c:set var="queryParams" value="${queryParams}&itemStatus=${itemStatus}" /></c:if>
            ...
            <div id="tab-variants" class="<%= activeTab.equals("variants") ? "" : "hidden" %>">
                <div class="bg-surface border border-outline-variant/50 rounded-lg overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="font-body-sm text-body-sm">
                                ...
                                <% if (products.isEmpty()) { %>
                                <tr>
                                    <td class="py-4 px-4 text-center text-on-surface-variant font-body-sm text-body-sm" colspan="6">
                    ...
                    <!-- Pagination Footer for Variants Tab -->
                    <div class="bg-surface px-4 py-3 border-t border-outline-variant/30 flex items-center justify-between">
                        <c:set var="varQueryParams" value="&tab=variants" />
                        <c:if test="${not empty searchInput}"><c:set var="varQueryParams" value="${varQueryParams}&searchInput=${searchInput}" /></c:if>
                        <c:if test="${not empty category}"><c:set var="varQueryParams" value="${varQueryParams}&category=${category}" /></c:if>
                        <c:if test="${not empty sortBy}"><c:set var="varQueryParams" value="${varQueryParams}&sortBy=${sortBy}" /></c:if>
                        <c:if test="${not empty stockStatus}"><c:set var="varQueryParams" value="${varQueryParams}&stockStatus=${stockStatus}" /></c:if>
                        <c:if test="${not empty itemStatus}"><c:set var="varQueryParams" value="${varQueryParams}&itemStatus=${itemStatus}" /></c:if>

                        <c:choose>
                            <c:when test="${currentPage > 1}">
                                <a href="?page=${currentPage - 1}${varQueryParams}" class="px-3 py-1 border border-outline-variant rounded text-on-surface-variant font-label-md text-label-md hover:bg-surface-container-low">
                                    Trước
                                </a>
                            </c:when>
                            <c:otherwise>
                        ...
                        <div class="flex gap-1 items-center flex-wrap">
                            <c:choose>
                                <c:when test="${totalPages <= 5}">
                                    <c:forEach begin="1" end="${totalPages}" var="i">
                                        <a href="?page=${i}${varQueryParams}" class="w-8 h-8 flex items-center justify-center rounded font-label-md text-label-md ${currentPage == i ? 'bg-primary text-on-primary' : 'text-on-surface hover:bg-surface-container-low'}">${i}</a>
                                    </c:forEach>
                                </c:when>
                                <c:otherwise>
                                    <!-- 3 trang đầu tiên -->
