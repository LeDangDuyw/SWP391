...
from spacy_nlu import is_allowed_domain # chỉ dùng kiểm tra domain cho phép
...
# LLM phụ chuyên dụng để phân tích NLU thuần túy (Cách 3)
llm_nlu = ChatOpenAI(
    model=os.getenv("LMSTUDIO_MODEL"),
    base_url=os.getenv("LMSTUDIO_BASE_URL"),
    api_key="lm-studio",
...
# =========================================================
# Phân tích NLU bằng Local LLM tối ưu (JSON Mode)
# =========================================================
...
def laptop_advisor_node(state: AgentState):
    ...
    # =========================================================
    # CHẠY 100% TRÊN LLM NLU (Bỏ qua hoàn toàn SpaCy)
    # =========================================================
    print(f"\n[PURE LLM NLU TRIGGERED] Gọi Local LLM NLU cho câu hỏi: '{query_to_analyze}'")
    nlu_result = analyze_with_llm_optimized(query_to_analyze)
